...
from chtorch.configuration import get_cfg_defaults
...
def run_pbmohpo_bench(
    ...
    if config.REPL.SEEDREPL:
        logging.info(f"Setting all random seeds to {config.REPL.SEEDREPL}")
        import random

        import numpy as np
        import torch

        random.seed(config.REPL.SEEDREPL)
        np.random.seed(config.REPL.SEEDREPL)
        torch.manual_seed(config.REPL.SEEDREPL)
        torch.cuda.manual_seed(config.REPL.SEEDREPL)
        torch.cuda.manual_seed_all(config.REPL.SEEDREPL)
    ...
    if config.PROBLEM.PROBLEM_TYPE == "TuneDeepAR":
        logging.info("Testing TuneDeepAR")
        prob = TuneDeepAR(data_path = config.PROBLEM.DATAPATH)
    ...
    if save_archive:
        ...
        df["seedrepl"] = str(config.REPL.SEEDREPL)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="./experiment_configs/example_config.yaml",
        dest="config",
    )
